...
/**
 * Start the pg-boss queue with retry logic for container startup
 * (DNS may not be ready immediately when container starts).
 *
 * @param {string} databaseUrl — PostgreSQL connection string (DATABASE_URL)
 * @param {number} maxRetries — max connection attempts (default 3)
 */
export async function startQueue(databaseUrl, maxRetries = 3) {
  if (boss) return boss;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.error(`[queue] pg-boss start attempt ${attempt}/${maxRetries} failed:`, err.message);
      boss = null;
